...
#ifdef FIX_CLASS
// clang-format off
FixStyle(state/change/ksat,FixStateChangeKsat);
// clang-format on
#else

#ifndef LMP_FIX_STATE_CHANGE_KSAT_H
...
class FixStateChangeKsat : public Fix {
 ...
 private:
  ...
  int group_patches;            // Group ID for all patches (types 2, 3, 4)
  int patch_mask;               // Cached bitmask for patches group (set in init())
  
  int *last_change;             // Per-atom: timestep of last change (for cooldown)
  int *effective_type;          // Per-atom: effective type (2, 3, or 4 for patches)
  double *prev_coord_type2;     // Per-atom: previous coordination with type 2
  ...
  double get_coordination(int, int); // Get coordination with target type for atom i
};
